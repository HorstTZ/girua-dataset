...
```{r}
# ASR: Não entendi a função desse peddaço de código.
...
cobertura <- 
  ...
  # sp::spTransform(wgs84) # ASR: Não entendo o que é feito aqui. O SRC é WGS84 e é transformado para WGS84!?
  sp::spTransform(wgs84utm21s) # ASR: os dados devem ser processados com SRC projetadas.
str(cobertura, 2)
```
...
str(pontos)
# pontos <- sp::spTransform(pontos, wgs84) # ASR: Os dados precisam ser projetados para a análise geoestatística
pontos@data
```
...
# ASR: Validação cruzada é muito demorada. Ademais, por que não usar o valor padrão de tuneLength (3)?
soc_rf <- caret::train(
  soc ~ elev + decli + vertnum + horiznum + uso, data = pontos@data, importance = TRUE)
varImp(soc_rf)
# ASR: por que não remover 'horiznum' do modelo?
```
...
```{r}
# ASR: Sempre é bom separar o código do processamento do código para geração de figuras.
...
# ASR: A função dist serve para distâncias no plano, não distãncias de arco.
distmax <- dist(pontos@coords) %>% max() / 4 # Por que 4???

# O valor predito de SOC pelo  modelo "soc_rf" foi o efeito fixo do Modelo Linear Misto de Variação Espacial 

limites <- seq(0, distmax, length.out = 10) # Por que só 10???
