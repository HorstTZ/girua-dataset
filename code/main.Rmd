...
```{r}
## carregar covariáveis preditoras
...
```{r}
# extrair valores da covariávies para os pontos
...
```{r}
soc_rf <- caret::train((soc ~ elev + decli + vertnum + horiznum + uso), data = pontos@data, method = "rf", tuneLength = 1, ntree = 100, importance = TRUE, na.action = na.omit, trControl = trainControl("LOOCV"))
